...
public partial class Database : MonoBehaviour
{
    ...
    class character_study : character_inventory
    {
    ...
    class character_bookskills
    {
        ...
        public int melee_combat { get; set; }
        public int ranged_combat { get; set; }
        public int survival { get; set; }
        public int tailoring { get; set; }
        public int unarmed_combat { get; set; }
    ...
    public void Connect()
    {
        ...
        connection.CreateTable<character_study>();
        connection.CreateIndex(nameof(character_study), new[] { "character", "slot" });
        ...
        connection.CreateIndex(nameof(character_traits), new[] { "character", "name" });
        connection.CreateTable<character_bookskills>();
        connection.CreateIndex(nameof(character_bookskills), new[] { "character", "name" });
    ...
    void LoadStudy(PlayerStudy study)
    {
        // fill all slots first
        for (int i = 0; i < study.size; ++i)
            study.slots.Add(new ItemSlot());

        // then load valid items and put into their slots
        // (one big query is A LOT faster than querying each slot separately)
        foreach (character_inventory row in connection.Query<character_study>("SELECT * FROM character_study WHERE character=?", study.name))
        {
            if (row.slot < study.size)
            {
                if (ScriptableItem.dict.TryGetValue(row.name.GetStableHashCode(), out ScriptableItem itemData))
                {
                    Item item = new Item(itemData);
                ...
                else Debug.LogWarning("LoadInventory: skipped item " + row.name + " for " + study.name + " because it doesn't exist anymore. If it wasn't removed intentionally then make sure it's in the Resources folder.");
            }
            else Debug.LogWarning("LoadInventory: skipped slot " + row.slot + " for " + study.name + " because it's bigger than size " + study.size);
        }
    }
    ...
    void LoadTraits(PlayerTraits traits)
    {
        character_traits row = connection.FindWithQuery<character_traits>("SELECT * FROM character_traits WHERE name=?", traits.name);

            //traits.agility = row.agility;
            //traits.awareness = row.awareness;
            //traits.charisma = row.charisma;
            //traits.intelligence = row.intelligence;
            //traits.resilience = row.resilience;
        
    }

    void LoadBookSkills(PlayerBookSkills bookskills)
    {
        character_bookskills row = connection.FindWithQuery<character_bookskills>("SELECT * FROM character_bookskills WHERE name=?", bookskills.name);
            //bookskills.academia = row.academia;
            //bookskills.agriculture = row.agriculture;
            //bookskills.animal_handling = row.animal_handling;
            //bookskills.athletics = row.athletics;
            //bookskills.cooking = row.cooking;
            //bookskills.crafting = row.crafting;
            //bookskills.firearms = row.firearms;
            //bookskills.leadership = row.leadership;
            //bookskills.mechanics = row.mechanics;
            //bookskills.melee_combat = row.melee_combat;
            //bookskills.ranged_combat = row.ranged_combat;
            //bookskills.survival = row.survival;
            //bookskills.tailoring = row.tailoring;
            //bookskills.unarmed_combat = row.unarmed_combat;
            //bookskills.vehicles = row.vehicles;
    ...
    public GameObject CharacterLoad(string characterName, List<Player> prefabs, bool isPreview)
    {
        ...
        if (row != null)
        {
            ...
            if (prefab != null)
            {
                ...
                player.combat.initialInvincibility            = row.remainingInv;
                ...
                LoadStudy(player.study);
                LoadTraits(player.traits);
                LoadBookSkills(player.bookskills);
    ...
    void SaveStudy(PlayerStudy study)
    {
        // inventory: remove old entries first, then add all new ones
        // (we could use UPDATE where slot=... but deleting everything makes
        //  sure that there are never any ghosts)
        connection.Execute("DELETE FROM character_study WHERE character=?", study.name);
        for (int i = 0; i < study.slots.Count; ++i)
        {
            ItemSlot slot = study.slots[i];
            if (slot.amount > 0) // only relevant items to save queries/storage/time
            {
                // note: .Insert causes a 'Constraint' exception. use Replace.
                connection.InsertOrReplace(new character_study
                {
    ...
    void SaveTraits(PlayerTraits traits)
    {
        connection.Execute("DELETE FROM character_traits WHERE character=?", traits.name);
            connection.InsertOrReplace(new character_traits
            {
                character = traits.name,

                agility = traits.agility,
                awareness = traits.awareness,
    ...
    void SaveBookSkills(PlayerBookSkills bookskills)
    {
        connection.Execute("DELETE FROM character_bookskills WHERE character=?", bookskills.name);
            connection.InsertOrReplace(new character_bookskills
            {
                character = bookskills.name,
                name = bookskills.name,
                academia = bookskills.academia,
            agriculture = bookskills.agriculture,
            animal_handling = bookskills.animal_handling,
            athletics = bookskills.athletics,
            cooking = bookskills.cooking,
            crafting = bookskills.crafting,
            firearms = bookskills.firearms,
            leadership = bookskills.leadership,
            mechanics = bookskills.mechanics,
            melee_combat = bookskills.melee_combat,
            ranged_combat = bookskills.ranged_combat,
            survival = bookskills.survival,
            tailoring = bookskills.tailoring,
            unarmed_combat = bookskills.unarmed_combat,
            vehicles = bookskills.vehicles
        });
    ...
    public void CharacterSave(Player player, bool online, bool useTransaction = true)
    {
        ...
        SaveStudy(player.study);
        SaveTraits(player.traits);
        SaveBookSkills(player.bookskills);
