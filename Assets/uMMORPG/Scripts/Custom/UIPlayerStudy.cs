...
public partial class UIPlayerStudy : MonoBehaviour
{
    public static UIPlayerStudy singleton;
    ...
    void Update()
    {
        ...
        if (player != null)
        {
            ...
            if (panel.activeSelf)
            {
                ...
                UIUtils.BalancePrefabs(slotPrefab.gameObject, player.study.slots.Count, content);

                // refresh all items
                for (int i = 0; i < player.study.slots.Count; ++i)
                {
                    UIInventorySlot slot = content.GetChild(i).GetComponent<UIInventorySlot>();
                    slot.dragAndDropable.name = i.ToString(); // drag and drop index
                    ItemSlot itemSlot = player.study.slots[i];
