...
public partial class NetworkManagerMMO : NetworkManager
{
    ...
    Player CreateCharacter(GameObject classPrefab, string characterName, string account, bool gameMaster)
    {
        ...
        for (int i = 0; i < player.study.size; ++i)
        {
            // add empty slot or default item if any
            player.study.slots.Add(i < player.study.defaultItems.Length ? new ItemSlot(new Item(player.study.defaultItems[i].item), player.study.defaultItems[i].amount) : new ItemSlot());
        }
