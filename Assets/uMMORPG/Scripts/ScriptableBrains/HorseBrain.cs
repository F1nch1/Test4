...
public class HorseBrain : CommonBrain
{
    [Header("Movement")]
    ...
    // states //////////////////////////////////////////////////////////////////
    string UpdateServer_IDLE(Monster monster, Player player)
    {
    ...
    string UpdateServer_MOVING(Monster monster, Player player)
    {
        // events sorted by priority (e.g. target doesn't matter if we died)
        if (EventDied(monster))
        {
    ...
    public override string UpdateServer(Entity entity)
    {
        ...
        if (monster.state == "IDLE") return UpdateServer_IDLE(monster, player);
        if (monster.state == "MOVING") return UpdateServer_MOVING(monster, player);
